...
function ImprovedProfileScreen() {
  const { user, logout } = useAuth();
  const { theme } = useTheme();
  const { trackNavigation, trackSettingsChange } = useEnhancedAnalytics();
  ...
  if (showPrivacySettings) {
    return <PrivacySettingsScreen onBack={() => setShowPrivacySettings(false)} />;
  }
      ...
      <ScrollView style={styles.container}>
        ...
        <View style={styles.optionsContainer}>
          ...
          <TouchableOpacity style={styles.optionItem} onPress={() => {
            trackNavigation('PrivacySettings', { source: 'profile_menu' });
            setShowPrivacySettings(true);
          }}>
            <Icon name="shield" size={24} color={theme.colors.text} />
