...
import { AnalyticsEvent } from '../contexts/EnhancedAnalyticsContext';
...
export class OfflinePersonalizationService {
  ...
  async storeOfflineAnalyticsEvent(event: AnalyticsEvent): Promise<void> {
    try {
      const existingEvents = await this.getOfflineAnalyticsEvents();
      existingEvents.push({
        ...event,
      ...
      console.error('Error storing offline analytics event:', error);
    }
  }

  // Get offline analytics events
  async getOfflineAnalyticsEvents(): Promise<any[]> {
    try {
      const eventsJson = await AsyncStorage.getItem('offline_analytics_events');
      return eventsJson ? JSON.parse(eventsJson) : [];
    } catch (error) {
      console.error('Error getting offline analytics events:', error);
      return [];
    }
  }

  // Sync offline analytics events
  async syncOfflineAnalyticsEvents(userProfileService: any): Promise<void> {
    try {
      const offlineEvents = await this.getOfflineAnalyticsEvents();
      ...
      for (const event of offlineEvents) {
        ...
        await userProfileService.storeAnalyticsEvent(eventWithDate);
      }

      // Clear offline events after successful sync
      await AsyncStorage.removeItem('offline_analytics_events');
